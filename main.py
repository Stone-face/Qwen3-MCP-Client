...
class Server:
    ...
    async def execute_tool(
        ...
        while attempt < retries:
            try:
                logging.info(f"Executing {tool_name} on server {self.name} with args {arguments}...")
                result = await self.session.call_tool(tool_name, arguments)
                logging.info(f"Result of {tool_name} on server {self.name} with args {arguments} : {result}")
                return result
            except Exception as e:
                attempt += 1
                logging.warning(
